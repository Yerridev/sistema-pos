...
class VentaViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        Caja = get_model('caja', 'Caja')
        MovimientoCaja = get_model('caja', 'MovimientoCaja')
        Venta = get_model('ventas', 'Venta')
        DetalleVenta = get_model('ventas', 'DetalleVenta')
        Producto = get_model('productos', 'Producto')

        caja_id = request.data.get('caja_id') or request.data.get('caja')
        detalles = request.data.get('detalles', [])
        metodo_pago = request.data.get('metodo_pago', 'EFECTIVO')
        descuento = Decimal(str(request.data.get('descuento', 0)))

        if not caja_id:
            return Response({'caja_id': 'La caja es obligatoria.'}, status=status.HTTP_400_BAD_REQUEST)
        if not detalles:
            return Response({'detalles': 'Debe enviar al menos un producto.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if caja.estado != 'ABIERTA':
            return Response({'caja_id': 'La caja debe estar ABIERTA para registrar ventas.'}, status=status.HTTP_400_BAD_REQUEST)
        if caja.cajero_id != request.user.id and request.user.rol != 'admin':
            return Response({'detail': 'No puede vender en una caja de otro cajero.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        for item in detalles:
            producto_id = item.get('producto') or item.get('producto_id')
    ...
    def anular(self, request, pk=None):
        ...
        if not motivo:
            return Response({'motivo': 'El motivo de anulación es obligatorio.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        MovimientoCaja.objects.create(
            ...
            tipo='EGRESO',
            monto=venta.total,
            concepto=f'Anulación venta #{venta.id}: {motivo}',
            usuario=request.user,
        )
